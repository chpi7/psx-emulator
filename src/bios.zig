...
pub const Bios = struct {
    ...
    pub fn load(filename: []const u8, allocator: std.mem.Allocator) !Bios {
        ...
        if (file_size != Bios.size) {
            log.err("Bios file {s} not of expected size (found = {}, expected = {})", .{ filename, file_size, Bios.size });
        }
        ...
        if (actually_read != file_size) {
            log.err("Bytes read vs file size mismatch! (expected {}, found {})", .{ file_size, actually_read });
        }

        log.debug("BIOS file {s} loaded (size = {}KB)", .{ filename, file_size / 1024 });
